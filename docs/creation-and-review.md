...
Parsing produces a local `PendingPortalImport`. The review lists subject/meeting totals, every subject, and every meeting. Warnings cover workbook or schedule data and are grouped under the affected subject code; complete enabled meetings are also compared immediately for overlaps. Every affected subject is marked with a red review indicator before confirmation. A skipped row without a code is labeled **Unidentified class**, while the original row-specific metadata remains available internally. Invalid meetings remain editable. Imported subjects use their Portal code as the sole identifier and neutral zero-unit representation. Each imported subject starts included and has an **Include in schedule** control. Exclusion preserves the complete pending subject and its imported metadata, removes its warnings and conflicts from the actionable review, and confirmation carries it into the project with `enabled: false`. Cancel discards only the pending object. Confirm creates/reuses a safe project and atomically replaces its schedule through the validated store action.
