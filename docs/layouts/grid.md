...
Phone Portrait intentionally has a narrower contract: Room and Time are available, while Professor and Section are unavailable. Its resolved defaults are Room on and Time off because position and height already communicate time. These Phone Grid preferences are stored per device variant and layout, so changing them does not rewrite Cards, Minimal, or larger-target Grid preferences. Roomy blocks may add an explicitly enabled Time; normal blocks show code and Room; constrained blocks retain only the complete subject code. Details are removed as whole fields when they do not fit—Phone Grid never renders an ellipsized time or room value.
