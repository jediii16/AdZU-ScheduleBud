...
A thin rounded vertical marker sits immediately beside the meaningful text height. Explicit single and per-subject colors remain exact. The automatic Minimal palette is a slightly more saturated companion to the pastel Cards palette so thin marks remain perceptible without becoming decorative. Marker widths remain 6 target px on Phone and portrait Tablet, 5 px on landscape Tablet and Square, and 4 px on Desktop. There are no filled class surfaces or separators between classes.
...
Portrait headings place a short blue rule beneath a dark weekday label. Wide landscape headings place the weekday beside a light continuation rule. Subject blocks are separated only by content-driven whitespace. A row's height is the tallest day section in that row; the next row starts after that height plus an intentional gap. The title, all day sections, and all class blocks form one compact movable group.

### Phone portrait

Packing is deterministic: six days use 2+2+2, five use 2+2+1, four use 2+2, three use 2+1, two use 2, and one uses a centered wider section. An incomplete last row is centered without a phantom slot. Four to six days retain the established 952 px working width on the 1080 × 2400 reference target. One to three days use a 992 px working width, making paired sections 20 px wider per column, while a singleton may grow to 700 px. The 42 px paired gutter and deliberate canvas margins remain intact.
...
Wide desktop/laptop targets use one column per visible day and center the entire grid. A day column is capped at **350 target px**, so sparse compositions become substantial without stretching absurdly. Five active days use most of the 1920 px design width with consistent gutters; six readable columns remain valid. The dark weekday label sits beside a restrained blue continuation rule.
...
Long professor and room/section values use a one-line fit with ellipsis. One long field never shrinks the entire layout. Minimal support text uses `#66758A`, modestly darker than the shared muted token while remaining subordinate to code and time. This token and the Minimal marker palette do not alter Cards.
...
`buildMinimalRenderModel()` is pure TypeScript. It resolves target family, active days, packing, text fitting, class height, row height, schedule bounds, and every text/line/marker coordinate before React Konva draws anything. Clean Slate supplies colors, font identities, and the subject palette; Minimal owns structure and geometry and contains no target coordinates in theme data.
